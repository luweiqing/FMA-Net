...
def load_mtb(fname, cat_id=1, **kwargs):
    ...
    df = pd.read_csv(
        ...
        header=None,
        names=['FrameId', 'Id', 'X', 'Y', 'Width', 'Height', 'Confidence', 'ClassId', 'SubClassId', 'unused1', 'unused2'],
        engine='python'
    )
    ...
    del df['unused1'], df['unused2']

    # Specific class
    df = df[df['ClassId'] == cat_id]

    # Remove all rows without sufficient confidence
    return df[df['Confidence'] >= min_confidence]


def load_motchallenge(fname, cat_id=1, **kwargs):
    r"""Load MOT challenge data.
...
def loadtxt(fname, cat_id=1, fmt=Format.MOT15_2D, **kwargs):
    """Load data from any known format."""
    fmt = Format(fmt)

    switcher = {
        Format.MTB_SAT: load_mtb,
    ...
    return func(fname, cat_id, **kwargs)
