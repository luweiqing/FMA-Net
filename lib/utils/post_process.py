...
import numpy as np
from lib.utils.image import transform_preds, get_affine_transform, transform_preds_with_trans,transform_preds_with_trans_gpu
...
def generic_post_process_N(dets, c, s, h, w):
  if not ('scores' in dets):
    return [{}], [{}]
  ret = []
  ...
  for i in range(len(dets['scores'])):  # batch 维度
    ...
    for j in range(len(dets['scores'][i])):  # topk 维度
      ...
      item['ind'] = dets['inds'][i][j]
      item['class'] = int(dets['clses'][i][j]) + 1
      item['ct'] = transform_preds_with_trans_gpu(
        (dets['cts'][i][j]).reshape(1, 2), trans).reshape(2)
      ...
      if 'bboxes' in dets:
        bbox = transform_preds_with_trans_gpu(
          dets['bboxes'][i][j].reshape(2, 2), trans).reshape(4)
        item['bbox'] = bbox
