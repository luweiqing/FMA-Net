...
def _topk_N(heatmap, K=40, num_classes=1):
    """
    ...
    # 计算每个类别对应的topk索引
    cls_inds_masks = torch.full((topk_clses.size(0), num_classes, topk_clses.size(1)), False, dtype=torch.bool,
                                device=topk_clses.device)

    for cls_id in range(num_classes):
        inds_masks = (topk_clses == cls_id)  # (N, K)
...
def ctdet_decode(heat, wh, reg=None, tracking=None, pre_inds=None, num_classes=1, cat_spec_wh=False, K=350):
...
def ctdet_decode_N(heat, wh, reg=None, tracking=None, pre_inds=None, num_classes=1, cat_spec_wh=False, K=350):
    # 作用是将heat_map解码成b-box 将输出转化成det的函数
    batch, cat, height, width = heat.size()
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    ...
    ret = {}
    # 2.1 之后经过_topk：
    scores, inds, clses, ys, xs, cls_inds_masks = _topk_N(heatmap=heat, K=K, num_classes=num_classes)
    if reg is not None:
        reg = _transpose_and_gather_feat(reg, inds)
        reg = reg.view(batch, K, 2)
        xs = xs.view(batch, K, 1) + reg[:, :, 0:1]
    ...
    detections = torch.cat([bboxes, scores, clses, inds], dim=2)
    # ret['bboxes'] = bboxes  # 当前帧出来的topk: B, K, 2
    # ret['scores'] = scores  # 当前帧出来的topk: B, K, 1
    # ret['clses'] = clses
    # ret['inds'] = inds  # 当前帧出来的topk: B, K
