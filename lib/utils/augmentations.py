...
class CropFixArea(object):
    ...
    def __call__(self, image, image_fg, boxes, labels, ids, pre_boxes, pre_ids):
        height, width, img_cha, seq_num = image.shape
        _, _, fg_cha, long_seq_num = image_fg.shape
        while True:
            if height < self.sample_area[1] or width < self.sample_area[0]:

                image_pad = np.zeros(
                    [max(height, self.sample_area[1]), max(width, self.sample_area[0]), img_cha, seq_num])
                image_fg_pad = np.zeros(
                    [max(height, self.sample_area[1]), max(width, self.sample_area[0]), fg_cha, long_seq_num])
            else:
                image_pad = image
                image_fg_pad = image_fg
            if height < self.sample_area[1] or width < self.sample_area[0]:
                for i in range(long_seq_num):
                    fg_bordered = cv2.copyMakeBorder(image_fg[:, :, :, i], 0,
                                                           max(0, self.sample_area[1] - height), 0,
                                                           max(0, self.sample_area[0] - width), cv2.BORDER_CONSTANT,
                                                           value=(0, 0, 0))
                    # 检查输出的形状
                    if fg_bordered.ndim == 2:
                        # 如果输出是二维的，将其扩展为三维 (H, W, 1)
                        fg_bordered = np.expand_dims(fg_bordered, axis=-1)
                    image_fg_pad[:, :, :, i] = fg_bordered
                    if i < seq_num:
                        img_bordered = cv2.copyMakeBorder(image[:, :, :, i], 0, max(0, self.sample_area[1] - height), 0,
                                                          max(0, self.sample_area[0] - width), cv2.BORDER_CONSTANT,
                                                          value=(0, 0, 0))
                        if img_bordered.ndim == 2:
                            img_bordered = np.expand_dims(img_bordered, axis=-1)
                        image_pad[:, :, :, i] = img_bordered
