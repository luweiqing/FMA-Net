...
class COCO(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = self.img_dir + temp_dir +'/' + imf + '/img1/' + imIdex + imtype
        ...
        for ii in range(seq_num):
            ...
            read_path = self.img_dir + temp_dir +'/' + imName
        ...
        # transpose
        inp = img.transpose(3, 2, 0, 1).astype(np.float32)  # img为输入的图像序列，将其转置为[seq_num, 3, im0.shape[0], im0.shape[1]]的浮点数数组作为输入。
