...
class COCO(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        temp_dir = self.split + '_data'
        img_path = self.img_dir + temp_dir + '/' + file_name
        ...
        for ii in range(seq_num):
            ...
            im = cv2.imread(self.img_dir + temp_dir + '/' + imName)
            #normalize
        ...
        for ratio in down_ratios:
            ...
            dis_map = np.zeros((seq_num - 1, 2, output_h, output_w), dtype=np.float32)
                    ...
                    for i in range(1, seq_num):
                        if i != seq_num - 1:
                            if pre_ids[i].count(obj_id) != 0 and pre_ids[i + 1].count(obj_id) != 0:
                                ...
                                # 更新 dis_map：在 temp_box_later 区域内，将所有位置的位移设为 ct_later - ct_pre
                                displacement = ct_later - ct_pre  # numpy array, shape (2,)
                                # 如果 displacement 非零（即目标发生了位移），则更新 dis_map 区域
                                if not np.allclose(displacement, 0):
                                    # 假设 temp_box_later 格式为 [x1, y1, x2, y2]
                                    x1 = int(temp_box_later[0])
                                    y1 = int(temp_box_later[1])
                                    x2 = int(temp_box_later[2])
                                    y2 = int(temp_box_later[3])
                                    # 在当前帧的 dis_map 区域内更新所有位置为 displacement
                                    dis_map[i - 1, 0, y1:y2, x1:x2] = displacement[0]
                                    dis_map[i - 1, 1, y1:y2, x1:x2] = displacement[1]
                        else:
                            if pre_ids[i].count(obj_id) != 0:
                                ...
                                # 更新 dis_map 对于最后一帧，同样在 temp_box_later 区域内写入位移
                                displacement = ct_later - ct_pre
                                if not np.allclose(displacement, 0):
                                    x1 = int(temp_box_later[0])
                                    y1 = int(temp_box_later[1])
                                    x2 = int(temp_box_later[2])
